...
def profile():
    ...
    if form.validate_on_submit():
        if User.authenticate(g.user.username, form.password.data):
            ...
            flash(f"Access unauthorized; incorrect password.", "danger")
            return redirect('/')
...
def like_message(msg_id):
    """Like a message if user is following message.user."""

    if not g.user:
        flash("Action unauthorized. You must be logged in to like messages.", "danger")
        return

    msg = Message.query.get(msg_id)

    if msg in g.user.likes:
        unlike = Likes.query.filter(Likes.user_id == g.user.id, Likes.message_id == msg.id).one()

        db.session.delete(unlike)
        db.session.commit()

        flash("Message removed from likes", "warning")
        return redirect(request.referrer)

    if msg.user != g.user and msg not in g.user.likes:
    ...
    flash("You can only like other user's messages.", "danger")
    return redirect(request.referrer)
